...
// Default: run a task. `vmmastra batch` tries a one-shot script first and
  ...
  async runRigCodeAct(prompt) {
    ...
    if (!llm || !guest) throw new Error('model or VM bridge is not ready');
    const messages = [
      { role: 'system', content: 'You are a coding agent working in /root/project on 32-bit Alpine Linux with BusyBox sh. Read /usr/local/share/vm-agent-capabilities.md for the canonical installed-tool and workflow reference. Installed commands include BusyBox utilities, jq, rg, git, curl, tar, gzip, qjs, vmjs, shfmt, ctags, make, patch, and vmproject; ShellCheck is not installed in this fixed-size i686 image. Format and verify shell scripts with shfmt and sh -n. Remote curl and git require a default route. AutoBro is not available in this Rig tool set. Accomplish the task by writing ONE POSIX sh script using the installed tools. Output ONLY the script body — no explanation and no markdown fences.' },
      { role: 'user', content: String(prompt) },
    ];
