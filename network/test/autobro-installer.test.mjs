...
test('Settings links the helper ZIP beside the extension', async () => {
  ...
  assert.match(html, /href="https:\/\/fapstaff\.com\/downloads\/autobro-web-bridge-2026\.07\.29\.4\.zip" download>Download AutoBro Chrome extension 2026\.07\.29\.4/);
...
test('extension source contains no Windows executable or PowerShell script', async () => {
  ...
  const files = await walk(new URL('autobro-extension-2026.07.29.4/', root));
  assert.deepEqual(files.filter((name) => /\.(?:exe|ps1)$/i.test(name)), []);
  const rootEntries = await readdir(new URL('autobro-extension-2026.07.29.4/', root));
  assert.ok(!rootEntries.includes('native'), 'extension must not contain a native directory');
  const zip = await readFile(new URL('downloads/autobro-web-bridge-2026.07.29.4.zip', root));
...
test('AutoBro calendar version is consistent', async () => {
  const publicVersion = '2026.07.29.4';
  ...
  assert.equal(manifest.version, '2026.7.29.4');
...
test('selected skills populate the automation prompt directly', async () => {
  const sourceDirectory = new URL('autobro-extension-2026.07.29.4/', root);
...
test('skill URLs normalize obsolete plu targets', async () => {
  const { normalizeSkillContent } = await import(
    new URL('../../autobro-extension-2026.07.29.4/src/skills.js', import.meta.url)
  );
