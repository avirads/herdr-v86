...
export function createVMAgent({ llmClient, guest, browserClient = null, onActivity = () => {}, approveAction = async () => false, sessionId = crypto.randomUUID() }) {
  ...
  const agent = createDeepAgent({
    ...
    skills: ['/skills/'],
    systemPrompt: {
      prefix: 'Work autonomously on the project using Deep Agents planning, filesystem, shell, and delegation tools. Read /usr/local/share/vm-agent-capabilities.md at startup for the canonical installed-tool and workflow reference. Inspect before editing, make focused changes, run relevant verification, and report evidence. Mutations and shell commands require user approval at execution time. Installed guest commands include BusyBox utilities, jq, rg, git, curl, tar, gzip, qjs, vmjs, shfmt, ctags, make, patch, and vmproject; ShellCheck is not installed in this fixed-size i686 image. Format and verify shell scripts with shfmt and sh -n, and use vmproject import/export for project archives. When AutoBro is connected, use autobro_automate for natural-language browser tasks so the page-local WebGPU LLM plans commands from exact live-page controls and skills. When that tool returns AUTOBRO_EXECUTION_COMPLETE, do not invoke any browser tool again for the same task; report its step results and final page directly in the vmlang shell. Use browser_search for explicit web searches, autobro_command only when the exact low-level command is already known, and vmfetch only for CORS-enabled HTTP APIs/resources. Provider tools automatically switch between equivalent vm* and AutoBro capabilities on recoverable failures; inspect switchedProvider results and continue with the active provider.',
      suffix: 'The backend maps Deep Agents path / to the real guest workspace /root/project. The guest is Alpine Linux i386 with BusyBox sh. Remote curl and git require a default route; without one use vmfetch or vmgithub archive. Do not claim success without reading results and running proportionate checks.',
    },
  });
  return {
    agent,
