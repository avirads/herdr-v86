...
function createTools({ guest, onActivity }) {
  ...
  return [
    createTool({
      ...
      async execute({ path }) { activity('read_file', { path }); return await guest.read(path); },
    }),
    createTool({
    ...
    createTool({
      ...
      async execute({ path, content }) { activity('write_file', { path }); await guest.write(path, content); return `wrote ${path}`; },
    }),
    createTool({
...
export function createClineVMAgent({
  ...
  return {
    ...
    async run(task) { return await runtime.run(String(task)); },
