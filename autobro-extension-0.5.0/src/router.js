...
import { getSkill, listSkills, loadSkills, importSkill, setSkillLoaded } from './skills.js';
...
async function bridgeCommand(payload) {
  switch (payload?.command) {
    ...
    case 'skillsList':
      return await listSkills(payload.maxChars ?? 0);
    case 'skillsLoad':
    case 'skillsUnload': {
      if (typeof payload.path !== 'string' || !payload.path) {
        throw new Error(`${payload.command} requires {path}`);
      }
      const result = await setSkillLoaded(payload.path, payload.command === 'skillsLoad');
      const loaded = await loadSkills('', 10000, 0);
      await setGuidewireCommandsEnabled(loaded.some(skill => isGuidewireSkillPath(skill.path)));
      return result;
    }
    default:
      return undefined;
