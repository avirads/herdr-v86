...
// Streaming chat port for the panel UI: one 'chat' message in, many
// {type:'chunk'} out, then {type:'done'} or {type:'error'}.
chrome.runtime.onConnect.addListener(port => {
  if (port.name !== 'web-bridge-ui-stream') return;
  port.onMessage.addListener(async message => {
    if (message?.type !== 'chat') return;
    try {
      await webgpuLlm.chatStream(message.body || {}, text => {
        try { port.postMessage({ type: 'chunk', text }); } catch { /* panel closed */ }
      });
      try { port.postMessage({ type: 'done' }); } catch { /* panel closed */ }
    } catch (error) {
      try { port.postMessage({ type: 'error', error: error?.message || String(error) }); } catch { /* panel closed */ }
    }
  });
});
