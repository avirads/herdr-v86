...
- **Step links** are exact ids (`<Wizard>-LOBWizardStepGroup-<Step>`); a
  ...
  id.
- **Session/bridge resilience**: check the extension panel Status; if Herdr is
  disconnected, reconnect it with the pairing token; if logged out, fill the
  login controls on the PolicyCenter tab using only user-supplied credentials.
